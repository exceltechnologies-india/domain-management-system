...
// ─── Old credit-note flags (history only since 26 Sep 2026) ─────────────────
// Nothing sets `creditNotePending` any more (owner: "those are only 'test
// orders' so no need for credit note"). The reader stays so integration-health
// lists rows flagged before, until an admin clears them. These tests set the
// flag directly, the way those old rows carry it.
describe("listCreditNotePendingOrders (old flags)", () => {
  ...
  it("lists only flagged orders", async () => {
    ...
    await flagAsOldRow(flagged._id, {
      refundId: "rfnd_l",
  ...
  it("lists OLDEST first", async () => {
    ...
    await flagAsOldRow(older._id, { refundId: "r1", refundAmountPaise: 100 });
    await flagAsOldRow(newer._id, { refundId: "r2", refundAmountPaise: 100 });
    await Order.updateOne(
      { _id: older._id },
  ...
  it("excludes soft-deleted orders", async () => {
    ...
    await flagAsOldRow(order._id, { refundId: "r", refundAmountPaise: 100 });
    await Order.updateOne({ _id: order._id }, { $set: { isDeleted: true } });
  ...
  it("clearing the flag removes it from the report", async () => {
