...
| Batch 7bb | M14 continued — `<ProfileCompletionWarning>` (12): show/hide gating, session+local merge precedence, missing-field grammar (singular vs "X and Y are"), trim check, Complete-now redirect with/without returnUrl, Dismiss hide, profileUpdated event-driven re-check | 🔄 Committed, deploy pending | `e243609` |
| Batch 7bc | M14 continued — `<HostingUpsell>` (5): card render (heading + ₹/strike-through/Save-50%/features), add-to-cart CartItem shape (period 12 + yearly + hostingPlan.id), auto-link to existing domain via `linkedDomain`, dedupe with error toast when same plan already in cart, legacy-domain-no-itemType still auto-links | 🔄 Committed, deploy pending | `77c745a` |
| Batch 7bd | M14 continued — `<DomainCrossSell>` (8): disabled-when-empty submit, no-dot auto-append `.com`, available/taken/success-false panels, apiClient-failure error toast, exact-match preference, typing clears result, add-to-cart CartItem shape (period 12, itemType domain) | 🔄 Committed, deploy pending | `ea272ac` |
| Batch 7be | M14 continued — `<DomainSetup>` (11): heading + plan-name fallback; Link tab — required + invalid-format errors, typing clears error, valid-link 500ms-delay onUpdateDomain + success toast; Buy tab — tab switch, .com auto-append, available/taken panels, apiClient failure, Add & Link fires both onAddDomainToCart (period 1) + onUpdateDomain and clears search state | 🔄 Committed, deploy pending | — |

**All four HIGHs + 11 MEDIUMs + 11 LOWs + 2 architectural suggestions (S2 + S3) cleared, M6 / L12 / M14 in progress; S1 effectively closed.** 63 vertical slices shipped across batches 7a–7be (6 RC ops + 6 DA ops + vocab unification + M13 cleanup + L1 Razorpay typed-client + nine M14 slices + L8 error boundaries + L12 jsx-a11y lint + M6 leaf demotion + S2 pre-commit hooks + S3 complete twelve-slice sweep + twenty-four S1 frontend-apiClient slices). Bonus catches:
