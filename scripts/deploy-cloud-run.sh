...
# COMPANY_STATE — the state our GSTIN is registered in. The GST engine needs it
# to choose CGST+SGST (same state) vs IGST (different state), and it REFUSES to
# issue an invoice without it (lib/billing/companyProfile.ts). It was called
# ZOHO_ORG_STATE until Zoho Books was removed on 24 Sep 2026.
#
# Preserved from the running service, like the flags above, so a deploy from a
# shell that lacks it does not blank it. And refused when empty: a deploy that
# leaves it unset would make every paid order fail to invoice, which is far
# worse than a deploy that does not happen.
CURRENT_COMPANY_STATE=$(read_current_env_var "COMPANY_STATE")
COMPANY_STATE="${CURRENT_COMPANY_STATE:-${COMPANY_STATE:-}}"
if [ -z "${COMPANY_STATE}" ]; then
  echo "❌ COMPANY_STATE is not set on the service or in this shell."
  echo "   Why: the GST engine cannot tell CGST/SGST from IGST without it, so every invoice would fail."
  echo "   Fix: export COMPANY_STATE=Delhi   (the state of GSTIN 07ABDCA0298H1ZP), then re-run this script."
  exit 1
fi
echo "   COMPANY_STATE resolved to: ${COMPANY_STATE}"
...
# and would otherwise be wiped back to the code default by the next deploy.
# Empty = unset = the [24, 72, 168] default in config/automation.ts.
CURRENT_RENEWAL_DUNNING_HOURS=$(read_current_env_var "RENEWAL_DUNNING_HOURS")
RENEWAL_DUNNING_HOURS="${CURRENT_RENEWAL_DUNNING_HOURS:-${RENEWAL_DUNNING_HOURS:-}}"
