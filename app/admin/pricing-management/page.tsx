  ...
  const columns = [
    {
      ...
      render: (value: string) => (
        <div className="flex items-center">
          <Globe className="h-4 w-4 text-amber-ink mr-2" />
    ...
    },
    {
      ...
      render: (value: number, row: TLDPricing) => (
        <div className="flex items-center">
          ...
          <span className="text-sm text-ink-3 ml-1">{row.currency}</span>
        </div>
      )
    },
    {
      ...
      render: (value: number, row: TLDPricing) => (
        <div className="flex items-center">
          <TrendingUp className="h-4 w-4 text-amber-ink mr-1" />
          <span className="font-semibold text-ink">
            {formatIndianCurrency(value)}
          </span>
          <span className="text-sm text-ink-3 ml-1">{row.currency}</span>
    ...
    },
    {
      ...
      render: (value: string) => (
        <span className="text-sm text-ink-2">{value || 'N/A'}</span>
      )
    }
  ];
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start sm:items-center justify-between flex-col sm:flex-row gap-3 sm:gap-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-soft rounded-xl">
              <Tag className="h-5 w-5 text-amber-ink" />
            </div>
            <div>
              ...
              <p className="text-sm text-ink-3 mt-0.5">
                ...
                {lastUpdated && !isDataLoading && (
                  <span className="text-ink-4"> · Updated {formatIndianDateTime(lastUpdated)}</span>
                )}
              </p>
            </div>
          </div>
        ...
        {!isCached && !isDataLoading && !isPurgingCache && tldPricing.length > 0 && (
          <div className="flex items-start gap-3 p-4 bg-indigo-soft border border-indigo/25 rounded-2xl">
            <CheckCircle2 className="h-4 w-4 text-indigo-ink mt-0.5 shrink-0" />
            <div>
              <p className="text-sm font-semibold text-indigo-ink">Fresh Data Loaded</p>
              <p className="text-xs text-indigo-ink/80 mt-0.5">
        ...
        <div className="bg-paper border border-hairline rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-hairline bg-paper-2/60 space-y-3">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2.5">
                <Tag className="h-4 w-4 text-ink-3" />
                <h3 className="text-sm font-semibold text-ink">TLD Pricing</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <div className="relative sm:w-44">
                <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-ink-4 pointer-events-none" />
                <select
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value)}
                  className="w-full pl-10 pr-3 py-2 text-sm bg-paper border border-hairline rounded-xl appearance-none focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition-shadow"
                >
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="px-3 py-2 text-sm bg-paper border border-hairline rounded-xl focus:outline-none focus:ring-2 focus:ring-amber focus:border-transparent transition-shadow"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center w-9 h-9 bg-paper border border-hairline rounded-xl hover:bg-paper-2 transition-colors text-ink-2"
                >
