    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-col sm:flex-row gap-3 sm:gap-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-soft rounded-xl">
              <Store className="h-5 w-5 text-amber-ink" />
            </div>
            <div>
              <h1 className="text-2xl font-bold font-serif text-ink">Resellers</h1>
        ...
        <div className="bg-paper rounded-xl border border-hairline p-5">
          <h2 className="text-lg font-semibold text-ink mb-4">Create a reseller</h2>
          <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label htmlFor="reseller-business" className="block text-sm font-medium text-ink-2 mb-1">
                Business name
              </label>
              <input
                ...
                className="w-full px-3 py-2 border border-hairline-strong rounded-md focus:outline-none focus:ring-2 focus:ring-amber"
              />
            </div>
            <div>
              <label htmlFor="reseller-email" className="block text-sm font-medium text-ink-2 mb-1">
                Owner email
              </label>
              <input
                id="reseller-email"
            ...
            <div>
              <label htmlFor="reseller-markup" className="block text-sm font-medium text-ink-2 mb-1">
                Markup % <span className="text-ink-4 font-normal">(optional)</span>
              </label>
              <input
                id="reseller-markup"
                type="number"
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-hairline text-sm">
              <thead className="bg-paper-2/60">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-ink-3">Business</th>
                  <th className="px-4 py-3 text-left font-medium text-ink-3">Owner</th>
                  <th className="px-4 py-3 text-left font-medium text-ink-3">Status</th>
                  <th className="px-4 py-3 text-left font-medium text-ink-3">Markup</th>
                  <th className="px-4 py-3 text-left font-medium text-ink-3">Created</th>
                  <th className="px-4 py-3 text-right font-medium text-ink-3">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-hairline">
                {loading ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-ink-4">Loading…</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-ink-4">No resellers yet.</td></tr>
                ) : (
                  rows.map((r) => {
                    ...
                    return (
                      <tr key={r._id}>
                        <td className="px-4 py-3">
                          <div className="font-medium text-ink">{r.businessName}</div>
                          <div className="text-xs text-ink-4">{r.slug}</div>
                        </td>
                        <td className="px-4 py-3 text-ink-2">{r.ownerUserId?.email || "—"}</td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${meta.classes}`}>
                            <Icon className="h-3 w-3" /> {meta.label}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-ink-2">{r.markupPercent}%</td>
                        <td className="px-4 py-3 text-ink-3">{formatDate(r.createdAt)}</td>
