...
export default function RcDiagnosticPage() {
  ...
  const Row = ({ label, value, mono = false }: { label: string; value: React.ReactNode; mono?: boolean }) => (
    <div className="flex items-start justify-between gap-4 py-2 border-b border-hairline last:border-0">
      <span className="text-xs font-medium text-ink-3 shrink-0">{label}</span>
      <span className={`text-sm text-ink text-right ${mono ? 'font-mono break-all' : ''}`}>{value}</span>
    </div>
  );
  ...
  return (
    <AdminLayout user={user || { firstName: 'Admin', lastName: '', email: '', role: 'admin' }} onLogout={performLogout}>
      <div className="space-y-6">
        ...
        <div className="bg-paper border border-hairline rounded-2xl shadow-sm p-4 sm:p-6">
          <label className="block text-xs font-semibold text-ink-3 uppercase tracking-wide mb-1.5">Domain</label>
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 border border-hairline rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-amber"
              />
            </div>
            <button
              onClick={() => void run()}
          ...
          <>
              ...
              <div className="flex-1">
                ...
                {!result.managedByThisAccount && (
                  <div className="mt-3">
                    ...
                    <p className="text-xs text-ink-3 mt-1.5">Soft-delete for domains transferred out / no longer managed here. Doesn&apos;t affect billing records or the registrar.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-paper border border-hairline rounded-2xl shadow-sm p-5">
                <h3 className="text-sm font-semibold text-ink mb-2">Stored in our records</h3>
            ...
            <p className="text-xs text-ink-4">Checked {new Date(result.checkedAt).toLocaleString('en-IN')}</p>
          </>
        )}
      </div>
    </AdminLayout>
