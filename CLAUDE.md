...
- **Hosting prices are ResellerOS's, not ours.** Owner: *"Use the prices of hosting set in ResellerOS completely. Ignore and disregard the prices of DMS from now on."* The source is `LANDING_PLANS` in the ResellerOS repo (`production/src/site/lib/data/hosting-landing.ts`). The prices in our `hostingplans` collection are no longer authoritative: do not build on them, "fix" them, or treat a mismatch with ResellerOS as ResellerOS's bug. **Not built yet:** the in-panel cart still prices from `hostingplans` and needs to read ResellerOS's prices instead.
