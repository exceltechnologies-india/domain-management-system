...
- ✅ **[H1] Order service-layer migration pass 2** (this pass) — Write-heavy Order paths now also go through the service: `payments/verify` (`getOrderByRazorpayOrderId` for idempotency check + new `forceMarkZohoCreationFailed`), `payments/guest/verify` (same), `workers/process-hosting-expiry` (`getOrderByOrderId` + new `createOrder`), `workers/sync-zoho-invoice` (collapsed to the existing `claimOrderForZohoInvoice`/`releaseZohoInvoiceClaim`/`recordZohoInvoiceForOrder`/`getOrderById` helpers — removed all four direct calls), `admin/hosting/provision` (`createOrder`), `domains/sync` + `domains/renew` (`createOrder`). New helpers: `createOrder`, `forceMarkZohoCreationFailed`. No remaining `Order.findOne` / `Order.find` / `Order.updateOne` / `Order.aggregate` / `Order.create` in `app/api/**`. Three `new Order(...)` constructor calls remain in transaction-bound write paths (`user/hosting/renew`, `user/hosting/upgrade`, `payments/guest/verify`) — deferred to pass 3 because they live inside Mongoose sessions that need a session-aware service helper.
...
#### [H1] Service-layer bypass — Order (pass 3) / Hosting / SupportTicket
**Files:** Passes 1+2 migrated 32 Order routes. Remaining work: 3 transaction-bound `new Order(...)` constructors (`user/hosting/renew`, `user/hosting/upgrade`, `payments/guest/verify`) — need a session-aware service helper before they can move; 22 Hosting routes; 2 SupportTicket routes.
**Problem:** HIGH-4 closed only `User`; the remaining models still have routes that call `.save()` / `Model.find` directly, bypassing the service layer.
**Fix:** Replicate the User-service migration pattern. The remaining Order writes need a `createOrderInSession(payload, session)` overload; Hosting/SupportTicket follow the same shape as the User and Order helpers.
