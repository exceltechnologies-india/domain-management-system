...
- ✅ **Rescan-3 Batch 6d (2026-05-21 lows)** — [L1] CSP nonce now generated from `crypto.getRandomValues(new Uint8Array(16))` then base64-encoded (Edge-runtime-safe equivalent of `randomBytes(16).toString("base64")`). Replaces the previous `Buffer.from(crypto.randomUUID()).toString("base64")` which encoded the 36-char UUID string with dashes — same entropy, longer output. [L2] Dead `localStorage`/`token`-cookie admin-auth fallback removed from 5 admin pages (`user-management`, `order-management`, `payment-management`, `pricing-management`, `invoices`). No auth route ever wrote those values; NextAuth session is the sole path now. Per-API-call `Authorization: Bearer ${token}` construction also removed (the token was always null at runtime; NextAuth cookies carry auth via `credentials:"include"`). Dropped unused `safeLocalStorage` imports. [L3] Rate-limiter comments reconciled: `userOrIpKey` helper now documents that no middleware emits `x-user-id`, so it currently IP-keys (callers needing genuine per-user limits use explicit `checkKey('prefix:' + user._id)`); `hostingRenewUpgrade` gained an `ipKey("hosting_renew_fallback")` keyGenerator as a safety net + comment clarifying callers MUST use explicit `checkKey`. [L4] `setupRcCustomerAndContact` no longer double-writes RC IDs — the redundant second `setUserResellerClubIds` call removed; in-memory `user.resellerClubCustomerId`/`resellerClubContactId` updates folded into the first write block. One `User.updateOne` round-trip instead of two. [L6] Daily-scheduler domain lock-step `findOneAndUpdate` now mirrors the candidate query's `status: { $nin: ["failed", "terminated"] }` filter, closing the rare race where a domain's status flips between fetch and lock acquisition. The Hosting branch already had this via `lockHostingForScheduler`. Tests: 426 unit + 159 integration = 585 green.
- ✅ **Rescan-3 Batch 6c (2026-05-21 medium / correctness + ops)** — [M1] Guest-verify catch fallback now checks if `orderId` already points at a persisted Order via `getOrderByOrderId`. If yes, skips the `createOrder` (was silently hitting the unique-index dupe). If no, creates the fallback with `status: "processing"` (not "completed") so the row-level status matches the inner `domains[].status: "pending"`. [M2] Webhook now passes `razorpay_signature: "webhook_verified"` to `finalizePendingOrder` instead of the "pending" placeholder — mirrors `webhook-handlers.ts:261` renewal-path sentinel; webhook-completed orders are now distinguishable from in-flight ones in the DB. [M3] Maintenance-mode middleware `fetch` gained `AbortSignal.timeout(2000)`; a hung `/api/public/maintenance-status` handler no longer blocks every middleware-handled request (existing catch already fail-opens). [M4] `serverLogger.remoteLog` `fetch` gained `AbortSignal.timeout(2000)`; recursion loop guard added at the other end — `/api/admin/log-error`'s catch now uses `console.error` instead of `serverLogger.error`, so a Mongo / SystemLog-model outage no longer turns a single failed write into an exponential request storm. [M5] `/admin/settings` step-up auth no longer trusts the body-supplied `category` field. New `isSecurityScopedKey(key)` helper checks the stored setting's category plus a server-side `SECURITY_KEYS` allowlist covering security-critical keys that may not exist yet (`2fa_required`, payment / DA / RC / Zoho / SMTP secrets, encryption keys). [M6] Remaining direct `Order.*` callsites outside `lib/services/orders.ts` migrated: webhook → `findOrderByRazorpayOrderIdOrInternalId` + `getOrderByRazorpayPaymentId` (new + existing service helpers); `lib/domain-verification.ts` → new `findOrdersByDomainName` helper; dead `import Order` lines removed from `admin/hosting/stats/route.ts` and `payments/guest/verify/route.ts`. Tests: 426 unit + 159 integration = 585 green.
- ✅ **Rescan-3 Batch 6b (2026-05-21 payment correctness)** — [H2] Zoho invoices now built from a DB-trusted projection of `order.domains` instead of the request body. New exported helper `cartItemsFromOrderDomains` in [order-creator.ts](lib/services/payment/order-creator.ts) used by both `/verify` and `/guest/verify` for the `createZohoInvoice` call. Closes the parallel of Batch 5a's [H1] for the financial-document path — invoice + GST record now match what was actually sold. [H3] `isTrial` round-trips: added to the `Order.domains` subschema + IOrder TS type, mirrored into the `domains.map` in both `app/api/payments/create-order/route.ts` and `app/api/payments/guest/create-order/route.ts`, propagated in `finalizePendingOrder`'s cartItems rebuild + the new `cartItemsFromOrderDomains` helper. Bundle carts (trial hosting + paid domain) no longer defeat the 1-trial-per-user eligibility gate. [H4] `handleVerificationError` now updates the existing pending Order via `Order.updateOne` instead of creating a duplicate fallback row. Preserves M4's post-provisioning state, keeps the real Razorpay tracking ids, and uses `status: "processing"` (not "completed") since no domain was registered. The legacy fallback path (no existing Order, defensive only) now also preserves real Razorpay ids when available. Caller in `/verify` hoists `razorpay_order_id`, `razorpay_payment_id`, and `existingOrder` to outer scope so the catch can pass them through. [L5] folded in: the catch-all branch in [verification-error.ts:194](lib/services/payment/verification-error.ts) no longer echoes raw `error.message` to the user response; raw message stays in `serverLogger.error`. Tests: 426 unit + 159 integration = 585 green.
...
_Batch 5b (webhook parity) closed: [H2], [M1], [L2], [L3]._
_Batch 5c (medium / quality) closed: [M2], [M3] (claim races + amount-mismatch; fan-out ordering + guest-409 covered by route paths, deferred to e2e), [M4], [M5]._
_Batch 5d (lows) closed: [L1], [L4]._

**Rescan 2026-05-21 post-batch is now fully closed.** Remaining open work is only the deliberately deferred key-rotation items below.
...
_Batch 6b (payment correctness) closed: [H2], [H3], [H4], [L5]._
_Batch 6c (medium / correctness + ops) closed: [M1], [M2], [M3], [M4], [M5], [M6]._
_Batch 6d (lows) closed: [L1], [L2], [L3], [L4], [L6]._

**Rescan-3 2026-05-21 is now fully closed.** Remaining open work is only the deliberately deferred key-rotation items below.
...
_Batch 3 (perf / latency) closed: [H5], [H6], [H7], [M8], [M9], [M10], [M13], [M14], [M15], [L10]._
_Batch 4 (quality / coverage) closed: [H3], [H4], [M6] (partial — helper added, 14 admin routes still on duplicate pattern as known follow-up), [M7] (payments + settings; 6 services still uncovered as follow-up), [M12], [L4], [L5], [L7], [L9]. [L11] dropped as not-actionable._
...
### ~~Batch 3 — perf / latency~~ ✅ shipped (see Resolved section)
### ~~Batch 4 — quality / coverage~~ ✅ shipped (see Resolved section)

### ~~Known follow-ups~~ ✅ both shipped

- ~~**[M6 sweep]** 14 admin routes still use the inline duplicate pattern.~~ ✅ All admin routes now call `AuthService.getAdminFromRequest`.
- ~~**[M7 expansion]** 6 service modules still untested.~~ ✅ New suites added: `system-logs`, `ip-checks`, `domain-watches`, `hosting-plans`, `renewal-payments`, `pending-domains`. **153 integration tests total**.

### Strengths to preserve

- Service-layer pattern in [lib/services/](lib/services/) for User / Order / Hosting / SupportTicket — every direct call in `app/api/**` routed through it (as of H1 + H4 close).
- 539 tests across unit + integration suites — runtime ~40s unit, ~24s integration.
