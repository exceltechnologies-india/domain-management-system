...
- **[M6 sweep]** 14 admin routes still use the inline `let user = AuthService.getUserFromRequest(...); if (!user) { getToken(...); }` duplicate pattern. `AuthService.getAdminFromRequest` exists; sweep is mechanical and can land as a single chore PR.
- **[M7 expansion]** 6 service modules still untested: `system-logs`, `domain-watches`, `ip-checks`, `renewal-payments`, `hosting-plans`, `pending-domains`. Each is small; add one suite each as time allows.
