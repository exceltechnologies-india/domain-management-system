...
#### [H1] Service-layer bypass — Order (pass 2) / Hosting / SupportTicket
**Files:** Pass 1 (above) migrated 25 read-side Order routes. Remaining work: 5 write-heavy Order routes (`payments/verify`, `payments/guest/verify`, `workers/process-hosting-expiry`, `workers/sync-zoho-invoice`, `admin/hosting/provision`); 22 Hosting routes; 2 SupportTicket routes.
...
**Fix:** Replicate the User-service migration pattern. Worker/payment routes need careful refactor — they own the critical mutate path so each one warrants its own commit + integration-test sweep.
